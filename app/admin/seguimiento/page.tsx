...
export default function SeguimientoPage() {
  ...
  // Controles de visibilidad independientes
  ...
  const inicializarDatos = async () => {
    // 1. Cargar Vendedores (Usando la lógica de tu reporte)
    const { data: emps } = await supabase.from('usuarios').select('*').order('nombre')
    if (emps) setVendedores(emps)

    // 2. Cargar Clientes (Usando 'nombre_local' como en tus archivos)
  ...
  const cargarDatos = async () => {
    ...
    if (visitasData) {
      // Cruzamos datos localmente para obtener nombres
  ...
  return (
    <div className="flex flex-col gap-4 max-w-lg mx-auto pb-24 px-2">
      <div className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-100 mt-2">
        <h1 className="text-xl font-black uppercase italic tracking-tighter mb-4">Seguimiento</h1>
