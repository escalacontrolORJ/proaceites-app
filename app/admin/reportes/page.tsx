...
export default function ReporteAdministrativo() {
  ...
  // --- FUNCIÓN WHATSAPP INTEGRADA ---
  const enviarWhatsApp = () => {
    if (filasFiltradas.length === 0) return alert("No hay datos para enviar")

    let mensaje = `*RESUMEN: ${tipoReporte.toUpperCase()}*\n`
    mensaje += `📅 ${fechaDesde} al ${fechaHasta}\n`
    mensaje += `--------------------------\n\n`

    if (tipoReporte === 'asistencia') {
      filasFiltradas.forEach(r => {
        mensaje += `👤 *${r.nombre}*\n`
        mensaje += `➡ Ent: ${r.entrada?.hora || '--'}\n`
        mensaje += `⬅ Sal: ${r.salida?.hora || '--'}\n`
        mensaje += `⏱ Horas: ${r.horas}h\n\n`
      })
    } else if (tipoReporte === 'visitas') {
      let total = 0
      filasFiltradas.forEach(r => {
        const v = parseFloat(r.valor_transaccion || 0)
        total += v
        mensaje += `🏢 *${r.nombre_cliente}*\n`
        mensaje += `💰 Valor: $${v.toFixed(2)}\n`
      ...
      filasFiltradas.forEach(r => {
        ...
        mensaje += `👤 Vend: ${r.nombre_vendedor}\n\n`
      })
    }
      ...
      <main className="p-4 md:p-8 max-w-7xl mx-auto">
        <div className="flex flex-col gap-6 bg-slate-900/50 p-6 rounded-3xl border border-white/5 mb-8 shadow-2xl">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={enviarWhatsApp} className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-lg flex items-center gap-2">
              WhatsApp
            </button>
            <button onClick={exportarExcel} className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-lg">Excel</button>
